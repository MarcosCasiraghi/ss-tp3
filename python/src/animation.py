...
def draw_new_frame(particle_data: [], obstacle_data: [], height: int, width: int, particle_radius:int, obstacle_radius: int, size_multiplier: int):
    frame = np.ones((height, width, 3), dtype=np.uint8) * 255
    for particle in particle_data:
        cv2.circle(frame, (int(particle[X] * size_multiplier), int(particle[Y] * size_multiplier)), radius= particle_radius,  # TODO: CHANGE
                   color=(0, 0, 255), thickness=-1)
    for obstacle in obstacle_data:
        cv2.circle(frame, (int(obstacle[X] * size_multiplier), int(obstacle[Y] * size_multiplier)), radius=obstacle_radius,  # TODO: CHANGE
...
def animate(particle_filename: str, static_data_filename: str, size_multiplier: int):
    ...
    fps = 30
    steps = 4
    particle_radius, obstacle_radius = int(static_data['pr'] * size_multiplier), int(static_data['or'] * size_multiplier)
    ...
    while i + 1 < len(particle_data):
        ...
        for p_i, o_i in zip(particle_intermediate_range, obstacle_intermediate_range):
            frame = draw_new_frame(p_i, o_i, height, width, particle_radius, obstacle_radius, size_multiplier)
            out.write(frame)
